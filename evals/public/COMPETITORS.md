...
|---|---|---|---|
| **codebase-memory-mcp** (DeusData) | ✅ **scored, live** | Local single-binary semantic code memory, headless CLI | None — see [`competitor.py`](competitor.py) + [`bench/public/competitor/`](../../bench/public/competitor/). Reproduce: `pip install codebase-memory-mcp==0.8.1 && python -m evals.public.competitor`. Capability comparison: [`docs/comparisons/vs-codebase-memory-mcp.md`](../../docs/comparisons/vs-codebase-memory-mcp.md) |
| **Graft** (trailhq) | ◐ **adaptable** | Two-pass local code graph (tree-sitter structural + optional LLM semantic pass), MIT, `@nanonets/graft` | Looks tractable: `graft ask "<question>" <dir>` is headless, returns ranked nodes with `file:line`, and the **structural** pass needs no API key or network — satisfying contract clauses 1–2 without paying for an LLM. Open questions a contributor must settle: whether to score structural-only (`graft build`) or `--deep` (LLM summaries, breaks the no-key property and pins a provider), and mapping its node ranking to a file ranking for `quality.py`. Capability comparison meanwhile: [`docs/comparisons/vs-graft.md`](../../docs/comparisons/vs-graft.md). Contribution-ready. |
