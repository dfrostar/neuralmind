...
def run_eval(query_filter: str | None = None, verbose: bool = False) -> dict[str, Any]:
    ...
    for i, query in enumerate(queries, 1):
        ...
        try:
            ...
            nm_tokens = nm_result.budget.total if hasattr(nm_result, "budget") else len(nm_context.split())
        ...
        if verbose or (i % 5 == 0):
            ...
            print(f"  [{status}] {query['id']}: overlap={scoring['max_overlap']:.2f}, tokens={nm_tokens}")
...
def print_report(report: dict[str, Any]) -> None:
    ...
    print(f"Hits (overlap >= 0.3): {report['hits']}/{report['total_queries']} ({report['recall_any_hit']:.1%})")
    print(f"Avg max overlap: {report['avg_max_overlap']:.3f}")
    print(f"Avg token reduction vs full-text: {report['avg_token_reduction']:.1f}×")
    print(f"Total tokens — Naive: {report['total_tokens_naive']:,} | NeuralMind: {report['total_tokens_nm']:,}")
    
    print("\nPer-query breakdown:")
    print("-" * 60)
    for r in report["per_query"]:
        status = "✓" if r["any_hit"] else "✗"
        print(f"  [{status}] {r['id'][:40]:40s} overlap={r['max_overlap']:.2f} reduction={r['reduction']:.1f}×")
