...
> **🆕 New in v0.20.0** — **Measure the onboarding lift.** `neuralmind eval --onboarding` turns NeuralMind's headline differentiator into a number: does an agent that inherits a **committed team memory** retrieve better on its first queries than a cold agent with none? A cold-vs-onboarded A/B over the gold queries, scored by the same offline judge as `neuralmind eval`, **budget-neutral by design**, and gated in CI at lift ≥ 0. The static code graph is commoditizing; the learned synapse layer is the moat. [Release notes](RELEASE_NOTES_v0.20.0.md)
>
...
| **[Release Notes v0.20.0](RELEASE_NOTES_v0.20.0.md)** | Onboarding-lift eval — `neuralmind eval --onboarding` measures the learned-synapse uplift (committed team memory vs a cold agent) as a cold/onboarded A/B over the gold queries; CI-gated at lift ≥ 0; `evals/onboarding/` mirrors the faithfulness eval |
