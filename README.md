...
> **🆕 New in v0.23.0** — **a versioned internal index contract (IR).** NeuralMind now builds a canonical, **schema-versioned** intermediate representation of your code graph and validates it on every build. A new command — **`neuralmind validate`** — checks that contract (dangling edges, orphaned nodes, unknown kinds, unsupported versions) **without a vector backend**, and `--write` migrates a legacy project's state in place (no rebuild). It's the first slice of the future-proofing roadmap's **PRD 1**: a **hidden-adapter, legacy-default** rollout that decouples retrieval/memory/UI/MCP from any one graph producer's field names. The embedder still reads `graph.json` exactly as before — the IR round-trips back to an identical graph — so retrieval and your existing index are unchanged. [Release notes](RELEASE_NOTES_v0.23.0.md)
>
...
| **[Release Notes v0.23.0](RELEASE_NOTES_v0.23.0.md)** | A versioned internal index contract (IR) — builds materialize a canonical, schema-versioned `IndexIR` and validate it; new `neuralmind validate` command (backend-free; reports dangling edges, orphaned nodes, unknown kinds, unsupported versions; `--write` migrates legacy state in place); round-trip-faithful graphify⇄IR adapter; IR metadata in `build`/`stats`; Phase-1 hidden-adapter rollout (PRD 1) — retrieval and existing indexes unchanged |
