...
NeuralMind requires a **code graph** to work. This is generated by [**graphify**](https://github.com/safishamsi/graphify), a separate tool that analyzes your codebase and creates a semantic graph.

**Graphify is not on PyPI**, so you must install from source:
...
**Note:** This only needs to run once per project. Re-run manually with `graphify build` after major code changes.
