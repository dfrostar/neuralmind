...
> **🆕 New in v0.22.0** — **ChromaDB-optional + auto backend.** `import neuralmind` no longer imports ChromaDB at all, and the default backend is now **`auto`**: it picks the ChromaDB-free **`turbovec`** path when its deps are installed (`pip install neuralmind[turbovec]`), else **chroma**. A bare install is unchanged; opting in is now effortless and the index migrates itself. Non-breaking by design — the hard default-flip waits until TurboVec has baked. [Release notes](RELEASE_NOTES_v0.22.0.md)
>
> **v0.21.0** — **ChromaDB-free retrieval.** The opt-in `turbovec` backend embeds *and* searches with **zero ChromaDB**: Google's **TurboQuant** compressed index (8–16× smaller vectors) + a bundled `OnnxMiniLMEmbedder` producing vectors **byte-identical** to ChromaDB's. At/above retrieval parity (fact-recall 0.744→0.800); retires the **CVE-2026-45829** dependency. [Release notes](RELEASE_NOTES_v0.21.0.md)
>
...
| **[Release Notes v0.22.0](RELEASE_NOTES_v0.22.0.md)** | ChromaDB-optional import + `auto` backend — `import neuralmind` no longer imports ChromaDB; default backend `auto` picks the ChromaDB-free `turbovec` path when its deps are installed (`neuralmind[turbovec]`), else chroma; self-healing reindex; non-breaking (bare installs unchanged); dropping chromadb from core deps staged for v0.23 |
