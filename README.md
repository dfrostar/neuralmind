...
> **🆕 New in v0.44.0 — Token receipts and batch cost estimates.** Two lightweight features that convert NeuralMind's existing token-savings data into dollar figures — pure math on logs you already have, no new I/O, no API calls. **(1) `neuralmind query --cost`** — adds a cost receipt line after every query: `Cost: $0.0036 used | $0.1308 saved (sonnet 3.00 $/MTok)`. Pick the model with `--model haiku/opus/gpt-4o` or override the price with `--rate`; `--json` adds `cost_used_usd`, `cost_saved_usd`, and `rate_per_mtok` to the output. **(2) `neuralmind batch-estimate`** — aggregates per-query token costs from the local event log and prints a cumulative cost table plus, with `--qpd N`, a monthly projection (`With NeuralMind: $11/mo · Without: $221/mo`). Same `--model`/`--rate` flags, same `--json` support. [Release notes](RELEASE_NOTES_v0.44.0.md)
>
> **v0.43.0** — **Surface what you can't see.** Three features, one thesis: the graph surfacing the thing a *flat list* hides. **(1) Decision provenance** — the *reason why*. A `Decision:` git trailer ("`resolveOrgId` is per-handler — avoids Prisma on `/health`, keeps tests simple") becomes a **subject-keyed, recallable memory**: `neuralmind why "why is resolveOrgId per-handler?"` returns the rationale with its commit SHA, and it injects on `UserPromptSubmit` alongside synapse recall (`NEURALMIND_PROVENANCE_INJECT`, on by default). Git history *is* the store — no DB to drift, works retroactively. **(2) Cohesion outlier detection** — the *odd one out*. When ten handlers route `orgId` through `resolveOrgId` and one skips it, `NEURALMIND_SYNAPSE_OUTLIERS=1` flags the member that breaks the cluster's shared pattern — the "handler #11" a flat recall list can't distinguish. **(3) `neuralmind gaps`** — the *coverage that lies*. Classifies endpoints as live-covered / mock-only / untested, so a route that's "green" only under a mocked store (the P2003 shape) stops reading as tested. [Release notes](RELEASE_NOTES_v0.43.0.md) **The code graph learns to answer "what does this touch?"** NeuralMind's recall has always run on two *soft* signals — semantic similarity and the learned synapse graph. This release adds the *hard* one the graph carried all along: **how the code is actually wired.** `graphify` already extracts typed structural edges (`calls`, `inherits`, `imports_from`) into every `graph.json`; until now they were dead weight. v0.42.0 turns them into an agent-visible capability — a new `neuralmind_structural_neighbors` MCP tool and `neuralmind structural <symbol>` CLI return a symbol's **callers, callees, base/sub classes, and importers**, and `--blast-radius` gives the transitive set of code a change would affect. Precise and available **day one** — the complement to the *learned* synapse layer: **structure says what *can* be related; synapses say what *actually* gets used together.** Folding structural neighbors into L3 retrieval is opt-in (`NEURALMIND_STRUCTURAL_RECALL=1`) so default retrieval stays byte-identical; the whole layer is killable with `NEURALMIND_STRUCTURAL=0`. [Release notes](RELEASE_NOTES_v0.42.0.md)
>
...
| **[Release Notes v0.44.0](RELEASE_NOTES_v0.44.0.md)** | **Token receipts and batch cost estimates** — pure math on existing token-savings data. **(1) `neuralmind query --cost`:** per-query cost receipt (`$used / $saved`) with model-slug or explicit $/MTok pricing, `--json` adds `cost_used_usd`/`cost_saved_usd`/`rate_per_mtok`. **(2) `neuralmind batch-estimate`:** aggregates the event log into a cost table + optional monthly projection (`--qpd N`) |
| **[Release Notes v0.43.0](RELEASE_NOTES_v0.43.0.md)** | **Surface what you can't see** — three features that make a flat list's blind spots visible. **(1) Decision provenance:** a `Decision:` git trailer becomes a recallable memory (`neuralmind why`, `NEURALMIND_PROVENANCE_INJECT`); git history is the store. **(2) Cohesion outlier detection:** flags the cluster member that skips a pattern its peers share, the "handler #11" (`NEURALMIND_SYNAPSE_OUTLIERS=1`). **(3) `neuralmind gaps`:** classifies endpoints live-covered / mock-only / untested, catching the P2003 "green only in mock mode" trap |
