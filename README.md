...
> **🆕 New in v0.46.0 — Audit trace over MCP.** CLI users have had `--trace`/`--explain` since PRD 3 shipped; agents talking to NeuralMind over MCP — the primary integration surface — couldn't ask "why did you give me this?" until now. `neuralmind_query(..., trace=true)` attaches the same structured `RetrievalTrace` the CLI produces — candidate pool, cluster scoring, synapse-boost attribution, final hits, token budget — as JSON on the response. Also closes the one asymmetry in the trace itself: L2 cluster-level synapse boosts were already traced, L3 hit-level boosts weren't; both are now attributed to the seed(s)/energy that produced them. Opt-in, defaults `false`, zero overhead and zero response-shape change for existing callers. No new storage — a trace lives for one query's lifetime, not a persistent audit log. [Release notes](RELEASE_NOTES_v0.46.0.md)
>
...
| **[Release Notes v0.46.0](RELEASE_NOTES_v0.46.0.md)** | Audit trace over MCP — `neuralmind_query(..., trace=true)` attaches the same structured retrieval trace (candidate pool, cluster scoring, synapse-boost attribution, final hits, token budget) the CLI's `--trace` has always produced, as JSON on the MCP response. Closes the CLI/MCP asymmetry and the L2/L3 synapse-boost tracing gap. Opt-in, no new storage — PRD 3 Phase 2 (MCP trace metadata) |
