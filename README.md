...
# code graph automatically (Python / TypeScript / Go / Rust / Java). No graphify needed.
neuralmind build .
...
| **[Release Notes v0.30.0](RELEASE_NOTES_v0.30.0.md)** | Team memory — commit your project's learned associations (`neuralmind memory publish` → `.neuralmind-team-memory.json`) and every teammate's agent inherits them automatically on its next session/build, imported once into the `shared` namespace (content-hash idempotent, `shared`-only, off-switch `NEURALMIND_TEAM_MEMORY=0`, fail-open). A fresh `git clone` starts with the team's earned intuition — zero setup, no source in the bundle, 100% local. The onboarding-lift eval measures the day-one head start. The differentiator a static code-index can't replicate |
