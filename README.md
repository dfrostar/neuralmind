...
### 2. Install Graphify (Required Dependency)

NeuralMind requires a **code graph** to work. This is generated by [**graphify**](https://github.com/dfrostar/graphify), a separate tool that analyzes your codebase and creates a semantic graph.
...
NeuralMind works with the code graph that graphify creates. Run this in your project:

```bash
cd /path/to/your/project
graphify update .
# Creates: graphify-out/graph.json (semantic analysis of your code)
```

**Note:** This only needs to run once per project. Graphify re-runs automatically on significant code changes, or manually with `graphify update .`.

### 4. Build the Neural Index

Now tell NeuralMind to build embeddings from the graph:

```bash
neuralmind build .
# Creates: .neuralmind/ (embeddings and audit trail)
```
...
If `Built: False`, make sure graphify created `graphify-out/graph.json`, then re-run `neuralmind build .`.

### Complete Setup (One Command)

For a new project, combine everything:

```bash
cd /path/to/your/project
pip install neuralmind graphify
graphify update . && neuralmind build .
neuralmind stats .  # verify
```
...
If `Built: False`, follow the [Installation & Setup](#-installation--setup) section above to initialize the project.
