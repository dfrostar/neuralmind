...
### `neuralmind mine-history` *(v0.42.0+)*

Warm up the synapse layer from git history. The associative memory normally
learns which files go together from live editing — which means it says little
until you've used it for a week. This mines the same signal from the commits that
already recorded it, so recall is useful from the first query.

```bash
neuralmind mine-history [project_path] [--max-commits N] [--max-files N] [--dry-run] [--json]
```

```bash
neuralmind mine-history .              # seed from the last 2000 commits
neuralmind mine-history . --dry-run    # preview the edges without writing
```
