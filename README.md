...
> **🆕 New in v0.27.0** — **Multi-language: Rust.** The built-in **tree-sitter** backend now indexes **Rust** out of the box — `neuralmind build .` works standalone on a Rust crate, no graphify. Structs, enums, traits, `impl` blocks, fields, and free functions become `code` nodes; `impl Trait for Type` becomes an **`inherits`** edge; `use` paths resolve to **`imports_from`** edges; doc comments (`///`, `//!`) become the **`rationale`** layer; and `target/` is skipped like `node_modules`. That takes the bundled backend to **four languages — Python, TypeScript, Go, and Rust** — indexed in one pass, all behind the same `SUPPORTED_SUFFIXES` seam. Proven at parity by the CI gate (100% symbol coverage on the reference fixture, zero dangling edges). [Release notes](RELEASE_NOTES_v0.27.0.md)
>
...
**No external tools required.** Since v0.15.0 a built-in **tree-sitter** backend indexes Python, TypeScript, Go, and Rust out of the box — `pip install neuralmind && neuralmind build .` just works, no `graphify` install. Optional extras:
...
# code graph automatically (Python / TypeScript / Go / Rust). No graphify needed.
neuralmind build .
