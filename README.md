...
> **🆕 New in v0.42.0** — **One-command setup: `neuralmind quickstart`.** Setup collapses from four commands to one: `quickstart` **builds the index**, **registers the MCP server** with every detected agent (Claude Code, Cursor, Cline, Claude Desktop, VS Code), **installs the Claude Code compression hooks**, and **runs `doctor`** to prove the result — a numbered `[1/4]…[4/4]` report of exactly what it did. Failure semantics are deliberate: only the build is fatal; the MCP and hook steps report and carry on, and the closing doctor verdict is quickstart's exit code. Every step stays available as its own command; flags: `--force`, `--global-hooks`, `--skip-build`/`--skip-mcp`/`--skip-hooks`. The docs now also lead with the **zero-install path** — `uvx neuralmind quickstart .` sets up a cold machine with no permanent install. [Release notes](RELEASE_NOTES_v0.42.0.md)
>
...
|---|---|---|
| **uvx (zero-install)** | `uvx neuralmind quickstart .` | Try it without installing anything — runs from an ephemeral env, prebuilt wheels, no compile step. `pipx run neuralmind …` works the same way. |
...
### Set up in one command *(v0.42.0+)*

```bash
neuralmind quickstart .
```

One command chains the whole setup: **build the index** → **register the MCP
server** with every detected agent (Claude Code, Cursor, Cline, Claude
Desktop, VS Code) → **install the Claude Code compression hooks** → **run
`doctor`** to verify, with a numbered report of each step. Only the build is
fatal; everything else reports and carries on, and the doctor verdict is the
exit code. `--skip-build` / `--skip-mcp` / `--skip-hooks` opt out of steps,
`--global-hooks` targets `~/.claude/settings.json`, `--force` rebuilds from
scratch. Each step remains available as its own command below.
...
```bash
# Install via any path above, then — the one-command path:
neuralmind quickstart .        # build + install-mcp + install-hooks + doctor
...
| **[Release Notes v0.42.0](RELEASE_NOTES_v0.42.0.md)** | One-command setup: `neuralmind quickstart` chains build → MCP registration (every detected agent) → Claude Code hooks → `doctor`, with a numbered `[1/4]…[4/4]` report. Only the build is fatal; the doctor verdict is the exit code. Flags: `--force`, `--global-hooks`, `--skip-build`/`--skip-mcp`/`--skip-hooks`. Plus a zero-install docs pass: `uvx neuralmind quickstart .` sets up a cold machine with nothing permanently installed |
