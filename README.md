...
> **🆕 New in v0.27.0** — **prove retrieval finds *your* code.** A new **`neuralmind probe`** command answers the question the other benchmarks couldn't: *on my codebase, can the agent actually find the right file when it asks about a symbol?* It runs **label-free** — no golden fixtures, no hand annotation — so it works on any built project: it samples indexed symbols, synthesizes a natural-language query from each symbol's identity (`authenticate_user` → `"authenticate user"`, never its node id), retrieves it back, and reports **recall@k / MRR / answerability** plus a **blind-spot list** naming the symbols your index can't find from a plain-English description. `benchmark` proves NeuralMind is *cheap* and `benchmark --quality` proves the ranking is good on a committed fixture; **`probe` proves it's good on *your* code** — and names what falls through. Borrows the long-context "needle" idea (S-NIAH in the *Recursive Language Models* paper) to measure whether the right node survives as the index grows. Read-only, deterministic per `--seed`, `--baseline` for before/after deltas, reuses the existing `neuralmind.quality` metric math. [Release notes](RELEASE_NOTES_v0.27.0.md)
>
> **v0.26.0** — **the selector starts tuning itself.** Phases 1–2 of the self-improvement engine (issue #156): NeuralMind's memory now logs query and wakeup events with a `session_id`, and an **opt-in tuner** reads that signal back to adjust the selector's **L2 recall depth** — how many community summaries a query surfaces. It's driven by the **re-query rate** (consecutive same-session queries whose recalled communities overlap heavily mean the first one under-disclosed and the agent had to come back), persisted in the synapse store's `meta` table, with a **transition-margin dampener** that suppresses a widen when the directional-transition graph already predicts the next hop decisively. **Off by default behind `NEURALMIND_SELECTOR_AUTOTUNE=1` — with the flag unset, behavior is byte-identical to v0.25.0** (zero extra hot-path I/O). With it on, L2 recall depth adapts to how often the agent had to re-query, bounded to `[2, 6]` with single-step, windowed, fail-open moves. Inspect it with the new read-only **`neuralmind self-improve status`**. [Release notes](RELEASE_NOTES_v0.26.0.md)
>
...
| **[Release Notes v0.27.0](RELEASE_NOTES_v0.27.0.md)** | Prove retrieval finds *your* code — a new label-free `neuralmind probe` command samples indexed symbols, synthesizes a natural-language query from each symbol's identity (`authenticate_user` → `"authenticate user"`, never its node id), retrieves it back, and reports recall@k / MRR / answerability plus a blind-spot list naming the symbols the index can't find from a plain-English description. Fills the gap between `benchmark` (proves it's cheap) and `benchmark --quality` (proves the ranking is good on a committed golden fixture): `probe` proves it's good on *your* codebase with no labeling. Borrows the long-context "needle" idea (S-NIAH, *Recursive Language Models*) to measure whether the right node survives as the index grows. Read-only, deterministic per `--seed`, `--baseline` for before/after deltas, `--json` for CI gating; new stdlib-only `neuralmind/probe.py` + `NeuralMind.retrieval_probe()`, reuses the `neuralmind.quality` metrics |
