...
[![Offline](https://img.shields.io/badge/Offline-100%20-blue.svg)](#-security--compliance)
...
| **Edit / Write** *(v0.41.0)* | New code is scanned for **reuse of existing graph symbols**; reused symbols co-activate in the synapse layer so retrieval learns what you actually reuse | — (learns, no token change) |

**This is fully automatic — you do not need to call any extra tools.**

**Reuse-vs-rewrite feedback *(v0.41.0+)*.** The `Edit`/`Write` PostToolUse hook is a pure side effect — it emits nothing to the agent and never forces a build. When new code references a symbol already defined elsewhere in your codebase, NeuralMind reinforces the synapse edge between the edited file and the reused definition, so future queries surface the code you've actually been *reusing*. The **implicit** complement to the explicit `neuralmind_feedback` MCP tool. Language-agnostic and fail-open; disable with `NEURALMIND_REUSE_FEEDBACK=0`.
...
and [Release Notes v0.24.0](https://github.com/dfrostar/neuralmind-marketing/blob/main/docs/RELEASE_NOTES_v0.24.0.md).
...
- **100% local, and ChromaDB-free by default** (v0.29.0) with byte-identical embeddings and an 8–16× smaller index.
- **It tunes itself locally.** v2.0 ships a population-based evolutionary tuner that proposes config changes, evaluates against real query traces, and promotes only if fitness beats incumbent by a hysteresis margin. No cloud, no telemetry.
...
- `graphify-out/neuralmind_db/` — vector store (created by `neuralmind build`; turbovec/ONNX by default, ChromaDB optional)
...
Build or incrementally update the neural index from the canonical IR (`.neuralmind/index_ir.json`) when present, falling back to `graphify-out/graph.json`.
...
agent and the codebase actually interact. See the [release notes](https://github.com/dfrostar/neuralmind-marketing/blob/main/docs/RELEASE_NOTES_v0.4.0.md) for the full story.
...
No. NeuralMind is fully offline — no API calls, no cloud services. Embeddings run locally via the default turbovec/ONNX backend (ChromaDB-free since v0.29.0); the knowledge graph is stored in your project. Query memory (optional, opt-in) is written to `.neuralmind/` on disk.
...
| **[Release Notes v0.19.0](https://github.com/dfrostar/neuralmind-marketing/blob/main/docs/RELEASE_NOTES_v0.19.0.md)** | One-command MCP setup — `neuralmind install-mcp --all` auto-detects Claude Code / Cursor / Cline / Claude Desktop and registers the MCP server with each (non-destructive merge, idempotent); pure-stdlib `mcp_install` module |
| **[Release Notes v0.18.0](https://github.com/dfrostar/neuralmind-marketing/blob/main/docs/RELEASE_NOTES_v0.18.0.md)** | Incremental per-file graph updates — `graphgen.update_files()` / `NeuralMind.update_files()` re-parse only changed files (unchanged files stay byte-identical so the embedder skips them), wired to the watcher via `neuralmind watch --reindex` |
| **[Release Notes v0.17.0](https://github.com/dfrostar/neuralmind-marketing/blob/main/docs/RELEASE_NOTES_v0.17.0.md)** | Optional SCIP precision pass — `NEURALMIND_PRECISION=1` folds compiler-accurate `calls`/`inherits` edges from a `*.scip` index into the built-in graph (replacing heuristic ones for covered files); off by default, dependency-free stdlib SCIP reader, proven by a precision check in the CI gate |
| **[Release Notes v0.16.0](https://github.com/dfrostar/neuralmind-marketing/blob/main/docs/RELEASE_NOTES_v0.16.0.md)** | Multi-language built-in backend — TypeScript + Go extractors behind the `SUPPORTED_SUFFIXES` seam, `neuralmind build` indexes Python/TS/Go with no graphify; proven at parity per language by the CI gate (100% symbol coverage on the reference fixtures) |
| **[Release Notes v0.15.0](https://github.com/dfrostar/neuralmind-marketing/blob/main/docs/RELEASE_NOTES_v0.15.0.md)** | Built-in tree-sitter graph backend — `neuralmind build` works with no graphify install; graphify still takes priority where present; backend swap proven at parity by a new CI gate (reduction + faithfulness within tolerance) |
| **[Release Notes v0.14.0](https://github.com/dfrostar/neuralmind-marketing/blob/main/docs/RELEASE_NOTES_v0.14.0.md)** | `neuralmind eval` — measure answer faithfulness (expected-fact recall vs a matched-budget naive baseline) with grounding + contradiction checks and `--json`; 100% local by default |
| **[Release Notes v0.13.0](https://github.com/dfrostar/neuralmind-marketing/blob/main/docs/RELEASE_NOTES_v0.13.0.md)** | Measurement foundation — offline faithfulness eval (query + gold-fact dataset, expected-fact-recall scorer), polyglot retrieval fixtures (TypeScript + Go), and a standard documentation process |
| **[Release Notes v0.12.0](https://github.com/dfrostar/neuralmind-marketing/blob/main/docs/RELEASE_NOTES_v0.12.0.md)** | Install Doctor — `neuralmind doctor` health checks (graph, index, synapses, MCP, hooks, memory), `--json` output, friendlier "graph not built" error |
| **[Release Notes v0.11.0](https://github.com/dfrostar/neuralmind-marketing/blob/main/docs/RELEASE_NOTES_v0.11.0.md)** | Directional Synapses — `synapse_transitions` table, `next_likely()` API, `neuralmind next` CLI, `neuralmind_next_likely` MCP tool |
| **[Release Notes v0.10.0](https://github.com/dfrostar/neuralmind-marketing/blob/main/docs/RELEASE_NOTES_v0.10.0.md)** | Agent Ergonomics — content-aware compression footer, `neuralmind last` recovery cache, small-failure passthrough |
| **[Release Notes v0.9.0](https://github.com/dfrostar/neuralmind-marketing/blob/main/docs/RELEASE_NOTES_v0.9.0.md)** | Enterprise-Ready — GHCR auto-build, CycloneDX SBOM, air-gapped install walkthrough, compliance one-pager |
| **[Release Notes v0.8.0](https://github.com/dfrostar/neuralmind-marketing/blob/main/docs/RELEASE_NOTES_v0.8.0.md)** | Always-On — systemd + launchd templates, Windows Task Scheduler walkthrough, `/healthz` endpoint |
| **[Release Notes v0.7.0](https://github.com/dfrostar/neuralmind-marketing/blob/main/docs/RELEASE_NOTES_v0.7.0.md)** | Install anywhere — `pip` / `pipx` / `uv` / Docker / source, Dockerfile, event-log rotation fix |
| **[Release Notes v0.6.0](https://github.com/dfrostar/neuralmind-marketing/blob/main/docs/RELEASE_NOTES_v0.6.0.md)** | Live activity feed, cross-process JSONL bridge, pin UX, depth slider, replay overlay |
