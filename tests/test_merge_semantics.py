...
from neuralmind.merge_semantics import QualityWeightedMerger, MergeConflict
...
class TestQualityWeightedMerger:
    def test_higher_quality_wins(self):
        ...
        hot_edge = scorer.score_edge(
            ...
            activation_count=50, created_at=time.time() - 86400,
            last_activated=time.time(), conflict_count=0, total_comparisons=10,
        )
        cold_edge = scorer.score_edge(
            source="a", target="b", namespace="personal",
            activation_count=1, created_at=time.time() - 86400 * 90,
            last_activated=time.time() - 86400 * 80, conflict_count=3, total_comparisons=5,
        )
    ...
    def test_tiebreaker_uses_activation_count(self):
        ...
        edge_a = EdgeQuality(
            source="x", target="y", namespace="shared",
            score=0.5, reinforcement_score=0.5, recency_score=0.5,
            conflict_rate=0.0, activation_count=100, age_days=1.0,
            should_promote=False, should_decay=False,
        )
        edge_b = EdgeQuality(
            source="x", target="y", namespace="personal",
            score=0.5, reinforcement_score=0.5, recency_score=0.5,
            conflict_rate=0.0, activation_count=200, age_days=1.0,
            should_promote=False, should_decay=False,
        )
