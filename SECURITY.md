...
10. **Dependencies.** Keep them updated:
    ...
    ```
    A **CycloneDX JSON** SBOM (`neuralmind-vX.Y.Z.sbom.json`, generated by Anchore `syft` over the full transitive tree) is served from [neuralmind.uk/sbom](https://neuralmind.uk/sbom) — see `.github/workflows/sbom.yml` and the [air-gapped install walkthrough](docs/use-cases/air-gapped.md).
