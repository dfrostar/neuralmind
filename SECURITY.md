...
10. **Dependencies.** Keep them updated:
    ...
    ```
    A **CycloneDX JSON** SBOM (`neuralmind-vX.Y.Z.sbom.json`, generated by Anchore `syft` over the full transitive tree) is generated for every release — see [neuralmind.uk/security](https://neuralmind.uk/security) for the current one, `.github/workflows/sbom.yml` for how it's built (GitHub Release attachment is best-effort and commonly falls back to a site-hosted copy, since release-please-created releases are immutable), and the [air-gapped install walkthrough](docs/use-cases/air-gapped.md).
