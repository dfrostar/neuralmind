...
def cmd_init_hook(args):
    ...
    if not os.path.exists(git_hooks_dir):
        print(f"Error: .git/hooks directory not found in {project_path}. Are you in a Git repository?")
        sys.exit(1)

    hook_path = os.path.join(git_hooks_dir, 'post-commit')

    hook_content = """#!/bin/bash
# Auto-generated by NeuralMind
echo "🧠 Synchronizing NeuralMind context index..."
graphify update . --quiet 2>/dev/null || true
neuralmind build . --quiet 2>/dev/null || true
echo "✓ NeuralMind index synced."
"""
    ...
    try:
        ...
        print(f"✓ Successfully installed post-commit hook at {hook_path}")
        print("NeuralMind will now automatically update in the background after every commit!")
    except Exception as e:
