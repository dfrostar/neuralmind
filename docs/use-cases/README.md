...
| **[Auditing retrievals over MCP](./auditing-retrievals-over-mcp.md)** | **Integrators wiring NeuralMind into another agent framework; anyone debugging a surprising result (v0.46.0+)** | **`neuralmind_query(..., trace=true)` returns structured, per-layer attribution for why a result was returned — the same trace `--trace` has shown on the CLI, now over MCP** |
