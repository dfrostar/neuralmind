...
from intel import (
    MARKET, COMPETITORS, VALUE_PROP_ENGINEERING, VALUE_PROP_BUSINESS,
    REFACTOR_PRIORITIES, FINANCIAL, PRICING, SEGMENTS
)
...
def style_cell(cell, text, font_size=11, color=WHITE, bold=False, fill=None, alignment=PP_ALIGN.LEFT):
    """Style a table cell."""
    cell.text = text
...
def add_footer(slide, text="NeuralMind — Confidential"):
    ...
    add_textbox(slide, Inches(0.3), Inches(7.1), Inches(12.7), Inches(0.3),
                text, font_size=8, color=MID_GRAY)
...
def build_deck():
    ...
    add_textbox(slide, Inches(1.2), Inches(2.0), Inches(11), Inches(0.8),
                "NeuralMind", font_size=52, color=WHITE, bold=True)
    add_textbox(slide, Inches(1.2), Inches(2.9), Inches(11), Inches(0.6),
                "Local-First Graph Intelligence for Engineering Teams",
                font_size=24, color=ACCENT_BLUE)
    add_textbox(slide, Inches(1.2), Inches(3.7), Inches(11), Inches(0.4),
                "Privacy-preserving AI that indexes your codebase — no code leaves your premise.",
                font_size=16, color=LIGHT_GRAY)

    add_textbox(slide, Inches(1.2), Inches(5.5), Inches(5), Inches(0.3),
                "Corporate Pitch Deck — July 2026", font_size=12, color=MID_GRAY)
    add_textbox(slide, Inches(1.2), Inches(5.9), Inches(5), Inches(0.3),
                "Confidential — For Investor Discussion", font_size=10, color=MID_GRAY)

    add_slide_number(slide, 1, total_slides)

    # ── SLIDE 2: Problem ──────────────────────────────────────────────────
    slide = prs.slides.add_slide(blank_layout)
    bg = add_shape(slide, 0, 0, SLIDE_W, SLIDE_H, fill=NAVY)

    add_shape(slide, Inches(0.5), Inches(0.3), Inches(0.08), Inches(0.5), fill=ACCENT_BLUE)
    add_textbox(slide, Inches(0.8), Inches(0.3), Inches(11), Inches(0.6),
                "Cloud AI Creates New Risks for Engineering Orgs", font_size=32, color=WHITE, bold=True)
    ...
    problems = [
        ("Data Sovereignty Risk", "Source code sent to third-party APIs — violating compliance requirements (SOC2, HIPAA, ITAR)."),
        ("Cost Escalation", f"Cloud AI pricing at $19–39/user/mo scales linearly — a 500-engineer org faces $1.5M+/yr in API costs."),
        ("Vendor Lock-in", "Proprietary models and formats create switching costs; code intelligence is trapped in one ecosystem."),
        ("Project Failure Rate", f"{MARKET['ai_project_cancellation_2027']:.0%} of AI projects will be cancelled by 2027 due to data governance and cost (Gartner)."),
        ("Knowledge Loss", "When engineers leave, cloud AI context leaves with them — no persistent institutional memory."),
    ]

    y = Inches(1.9)
    for title, desc in problems:
        add_shape(slide, Inches(0.8), y, Inches(0.06), Inches(0.5), fill=ACCENT_BLUE)
        add_textbox(slide, Inches(1.0), y, Inches(3.5), Inches(0.3),
                    title, font_size=14, color=ACCENT_BLUE, bold=True)
        add_textbox(slide, Inches(1.0), y + Inches(0.28), Inches(11.0), Inches(0.25),
    ...
    add_textbox(slide, Inches(1.0), Inches(1.82), Inches(5.4), Inches(0.35),
                "Engineering Value", font_size=16, color=ACCENT_BLUE, bold=True)
    ...
    for item in eng_items:
        add_textbox(slide, Inches(1.0), y, Inches(5.4), Inches(0.25),
    ...
    add_textbox(slide, Inches(7.0), Inches(1.82), Inches(5.4), Inches(0.35),
                "Business Value", font_size=16, color=GREEN, bold=True)
    ...
    for item in biz_items:
        add_textbox(slide, Inches(7.0), y, Inches(5.4), Inches(0.25),
                    f"▸  {item}", font_size=12, color=WHITE)
        y += Inches(0.42)
    ...
    for num, label, color in box_data:
        ...
        add_textbox(slide, x + Inches(0.2), Inches(1.45), Inches(3.1), Inches(0.6),
                    num, font_size=36, color=color, bold=True)
        add_textbox(slide, x + Inches(0.2), Inches(2.1), Inches(3.1), Inches(0.3),
                    label, font_size=12, color=LIGHT_GRAY)
        x += Inches(3.9)

    # Market details
    add_textbox(slide, Inches(0.8), Inches(3.1), Inches(11.5), Inches(0.4),
                "Key Market Dynamics", font_size=18, color=WHITE, bold=True)
    ...
    y = Inches(3.6)
    for d in dynamics:
        add_textbox(slide, Inches(1.0), y, Inches(11.3), Inches(0.28),
                    f"•  {d}", font_size=12, color=LIGHT_GRAY)
        y += Inches(0.42)
    ...
    # Pipeline steps
    steps = [
        ("1. Parse", "Multi-language graph parsing via tree-sitter/SCIP\n8000+ node capacity, incremental updates"),
    ...
    for title, desc in steps:
        ...
        # Title
        add_textbox(slide, x + Inches(0.2), Inches(2.0), Inches(2.4), Inches(0.4),
                    title, font_size=20, color=ACCENT_BLUE, bold=True)
        # Description
        add_textbox(slide, x + Inches(0.2), Inches(2.5), Inches(2.4), Inches(2.5),
    ...
    add_textbox(slide, Inches(0.8), Inches(5.8), Inches(11.5), Inches(0.3),
                "Key Differentiators", font_size=14, color=WHITE, bold=True)
    ...
    x = Inches(0.8)
    for d in diffs:
        add_textbox(slide, x, Inches(6.2), Inches(2.7), Inches(0.3),
                    f"✓ {d}", font_size=11, color=GREEN)
        x += Inches(2.9)
    ...
    for c, h in enumerate(headers):
        style_cell(tbl.cell(0, c), h, font_size=11, color=WHITE, bold=True,
                   fill=ACCENT_BLUE, alignment=PP_ALIGN.CENTER)
    ...
    for r, comp in enumerate(COMPETITORS, start=1):
        ...
        style_cell(tbl.cell(r, 0), comp["name"], font_size=10, color=name_color, bold=is_us, fill=row_fill)
    ...
    add_textbox(slide, Inches(0.8), Inches(5.9), Inches(11.5), Inches(0.3),
                "NeuralMind's Position", font_size=14, color=WHITE, bold=True)
    add_textbox(slide, Inches(0.8), Inches(6.25), Inches(11.5), Inches(0.4),
    ...
    for r, item in enumerate(REFACTOR_PRIORITIES, start=1):
        ...
        style_cell(tbl.cell(r, 1), item["impact"], font_size=11,
                   color=impact_colors.get(item["impact"], WHITE), bold=True,
                   alignment=PP_ALIGN.CENTER)
        style_cell(tbl.cell(r, 2), item["effort"], font_size=11,
                   color=effort_colors.get(item["effort"], WHITE), bold=True,
                   alignment=PP_ALIGN.CENTER)
    ...
    add_textbox(slide, Inches(0.8), Inches(5.5), Inches(11.5), Inches(0.3),
                "Immediate Actions", font_size=14, color=WHITE, bold=True)
    add_textbox(slide, Inches(0.8), Inches(5.85), Inches(11.5), Inches(0.4),
    ...
    for name, price, features, color in tiers:
        ...
        add_textbox(slide, x + Inches(0.2), Inches(2.0), Inches(3.3), Inches(0.35),
                    name, font_size=18, color=color, bold=True)
        add_textbox(slide, x + Inches(0.2), Inches(2.4), Inches(3.3), Inches(0.35),
                    price, font_size=22, color=WHITE, bold=True)
        add_textbox(slide, x + Inches(0.2), Inches(2.85), Inches(3.3), Inches(1.5),
                    features, font_size=11, color=LIGHT_GRAY)
        x += Inches(4.1)

    # Target segments
    add_textbox(slide, Inches(0.8), Inches(4.9), Inches(11.5), Inches(0.3),
                "Target Segments", font_size=14, color=WHITE, bold=True)

    seg_text = "  •  ".join([f"{s['name']}: {s['pain']}" for s in SEGMENTS])
    add_textbox(slide, Inches(0.8), Inches(5.25), Inches(11.5), Inches(0.8),
                seg_text, font_size=11, color=LIGHT_GRAY)
    ...
    for r, (metric, values) in enumerate(metrics, start=1):
        ...
        for c, val in enumerate(values, start=1):
            ...
            style_cell(tbl.cell(r, c), val, font_size=11, color=color,
                       alignment=PP_ALIGN.CENTER)

    # Key metrics summary
    add_textbox(slide, Inches(0.8), Inches(5.3), Inches(11.5), Inches(0.3),
                "Key Takeaways", font_size=14, color=WHITE, bold=True)
    add_textbox(slide, Inches(0.8), Inches(5.65), Inches(11.5), Inches(0.5),
    ...
    add_shape(slide, Inches(0.8), Inches(1.8), Inches(5.8), Inches(0.4), fill=DARK_GRAY)
    add_textbox(slide, Inches(1.0), Inches(1.82), Inches(5.4), Inches(0.35),
                "Hiring Plan (Year 1-2)", font_size=16, color=ACCENT_BLUE, bold=True)
    ...
    for role, detail in hires:
        add_textbox(slide, Inches(1.0), y, Inches(1.2), Inches(0.25),
                    role, font_size=11, color=ACCENT_BLUE, bold=True)
        add_textbox(slide, Inches(2.3), y, Inches(4.3), Inches(0.25),
    ...
    add_shape(slide, Inches(6.8), Inches(1.8), Inches(5.8), Inches(0.4), fill=DARK_GRAY)
    add_textbox(slide, Inches(7.0), Inches(1.82), Inches(5.4), Inches(0.35),
                "Use of Funds", font_size=16, color=GREEN, bold=True)
    ...
    for category, detail in funds:
        add_textbox(slide, Inches(7.0), y, Inches(1.8), Inches(0.25),
                    category, font_size=11, color=GREEN, bold=True)
        add_textbox(slide, Inches(8.9), y, Inches(3.7), Inches(0.25),
                    detail, font_size=11, color=LIGHT_GRAY)
        y += Inches(0.45)

    # Milestones
    add_shape(slide, Inches(0.8), Inches(4.5), Inches(11.5), Inches(0.05), fill=DARK_GRAY)
    add_textbox(slide, Inches(0.8), Inches(4.7), Inches(11.5), Inches(0.3),
                "12-Month Milestones", font_size=14, color=WHITE, bold=True)
    ...
    x = Inches(0.8)
    for m in milestones:
        add_textbox(slide, x, Inches(5.1), Inches(2.2), Inches(0.3),
                    f"◆ {m}", font_size=10, color=ACCENT_BLUE)
        x += Inches(2.3)
    ...
    add_textbox(slide, Inches(0.8), Inches(6.6), Inches(11.5), Inches(0.3),
...
if __name__ == "__main__":
    ...
    print(f"  Slides: 10")
    print(f"  Theme: Dark professional (navy/dark gray, white text, accent blue #0066CC)")
