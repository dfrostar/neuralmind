...
def build():
    ...
    add_table(
        ...
        [[c["name"], c["type"], c["strength"], c["weakness"], c["pricing"]] for c in COMPETITORS],
    )
    ...
    add_table(
        ...
        [[s["name"], s["pain"], s["fit"]] for s in SEGMENTS],
    )
    ...
    add_table(
        doc,
        ["Tier", "Price", "Features"],
        [[k.upper(), v["price"], v["features"]] for k, v in PRICING.items()],
    )
