...
| `NEURALMIND_ADVERSARIAL_RETRIEVAL` | `0` | *(v3.9.0+)* Opt-in (`== "1"`). Run the adversarial-retrieval pass on L3 — intent re-classification, code-signal boost, synapse-seeded expansion, and the two-pass source search that surfaces implementation files vector search misses. **Off by default**: as shipped it was net-negative on the faithfulness A/B (18 queries — 1 improved, 3 regressed) and grew the per-query slice ~22%, which, because the A/B scores NeuralMind against a *matched-budget* naive baseline, handed that baseline the same larger budget and took the gate from `+0.041` to `-0.065`. The code and its tests ship intact; the gate is the acceptance test for turning it on by default. |
