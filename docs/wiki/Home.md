...
| 🧠 | **Learns how you work** | A Hebbian *synapse* layer that learns co-edited files lifts top-k retrieval hit-rate — **+3.5 to +14 points across runs**, CI-gated on direction — **budget-neutral** (no extra tokens); on AVX-512 hosts it currently measures **-1.75 points** and the gate fails (open bug) | Synapse A/B eval (**reference fixture** — smaller scope) |
